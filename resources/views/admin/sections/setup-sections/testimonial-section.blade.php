        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#testimonial-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i> {{ __("Add Testimonial Item") }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                <td>
                                    <ul class="user-list">
                                        <li><img src="{{ get_image($item->image ?? "","site-section") }}" alt="testimonial"></li>
                                    </ul>
                                </td>
                                ...
                                @foreach ($languages as $item)
                                    ...
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif" id="edit-modal-{{ $item->name }}" role="tabpanel" aria-labelledby="edit-modal-{{$item->name}}-tab">
                                        <div class="form-group">
                                            @include('admin.components.form.input',[
                                                'label'     => __("Item Name").'*',
                                                'name'      => $lang_code . "_item_name_edit",
                                                'value'     => old($lang_code . "_item_name_edit",
                                                $data->value->language->$lang_code->item_name ?? "")
                                            ])
                                        </div>
                                        <div class="form-group">
                                            @include('admin.components.form.input',[
    ...
    <script>
        ...
        $(".edit-modal-button").click(function(){
            ...
            $.each(languages,function(index,item) {
                editModal.find("input[name="+item.code+"_item_name_edit]").val(oldData.language[item.code]?.item_name);
