...
@section('content')
        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#securityItem-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i> {{ __("Add Security Item") }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                <td>
                                    <ul class="user-list">
                                        <li><i class="{{ $item->language->$system_default_lang->item_section_icon ?? "" }}"></i></li>
                                    </ul>
                                </td>
                                <td>{{ $item->language->$system_default_lang->item_title ?? "" }}</td>
                                <td>
                                    <button class="btn btn--base edit-modal-button"  data-id="{{ $key }}"><i class="las la-pencil-alt"></i></button>
    ...
    <div id="securityItem-edit" class="mfp-hide large">
        <div class="modal-data">
            <div class="modal-header px-0">
                <h5 class="modal-title">{{ __("Edit Security Item") }}</h5>
            </div>
                                    ...
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif" id="edit-modal-{{ $item->name }}" role="tabpanel" aria-labelledby="edit-modal-{{$item->name}}-tab">
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.input',[
                                                ...
                                                'value'     => old($lang_code . "_item_description_edit",$data->value->language->$lang_code->item_description ?? "")
                                            ])
                                        </div>
                                        <div class="form-group">
                                            @include('admin.components.form.input',[
                                                'label'     => __("Item Section Icon").'*',
                                                'name'      => $lang_code . "_item_section_icon_edit",
                                                'value'     => old($lang_code . "_item_section_icon_edit",$data->value->language->$lang_code->item_section_icon ?? ""),
...
<script>
    openModalWhenError("securityItem-add","#securityItem-add");
    openModalWhenError("securityItem-edit","#securityItem-edit");
    ...
    $(".edit-modal-button").click(function(){
        ...
        var editModal = $("#securityItem-edit");
        ...
        $.each(languages,function(index,item) {
            ...
            editModal.find("input[name="+item.code+"_item_section_icon_edit]").val(oldData.language[item.code]?.item_section_icon);
        });

        openModalBySelector("#securityItem-edit");
