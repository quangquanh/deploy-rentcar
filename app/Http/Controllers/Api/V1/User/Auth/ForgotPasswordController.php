...
class ForgotPasswordController extends Controller
{
    public function findUserSendCode(Request $request)
    {
        ...
        return Response::success([__('Verification code sended to your email address')],['token' => $token,'code' =>$code ,'wait_time' => ""],200);
    }
    ...
    public function resendCode(Request $request)
    {
        ...
        if(Carbon::now() <= $password_reset->created_at->addMinutes(GlobalConst::USER_PASS_RESEND_TIME_MINUTE)) {
            return Response::error([__('You can resend verification code after ').Carbon::now()->diffInSeconds($password_reset->created_at->addMinutes(GlobalConst::USER_PASS_RESEND_TIME_MINUTE)). __(' seconds')],['wait_time' => (string) Carbon::now()->diffInSeconds($password_reset->created_at->addMinutes(GlobalConst::USER_PASS_RESEND_TIME_MINUTE))],400);
        }
        ...
        return Response::success([__('OTP resend success')],['token' => $validated['token'],'code' => $update_data['code'],'wait_time' => ""],200);
    }
